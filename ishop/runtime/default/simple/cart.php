	...
	<table width="100%" class="cart_table m_10">
		...
		<tbody>
			...
			<tr>
				...
				<td class="sell_price"id="<?php echo isset($item['sell_price'])?$item['sell_price']:"";?>" ><b>￥<?php echo isset($item['sell_price'])?$item['sell_price']:"";?></b></td>
			...
			<tr class="stats">
				<td colspan="8">
					...
					<font color="red">满三件减一件</font>金额总计（不含运费）：￥<b class="orange" id='sum_price'><?php echo isset($this->final_sum)?$this->final_sum:"";?></b>
				</td>
			</tr>
		</tbody>
		<tfoot>
		...
		$.getJSON("<?php echo IUrl::creatUrl("/simple/joinCart");?>",{"goods_id":goods_id,"type":goods_type,"goods_num":diff,"random":Math.random()},function(content){
			...
			else
			{
				...
				$.getJSON("<?php echo IUrl::creatUrl("/simple/promotionRuleAjax");?>",{"goodsId":goodsId,"productId":productId,"num":num,"random":Math.random()},function(content){
                    ...
                    $('#discount_price').html(content.reduce);//商品优惠
                    $('#sum_price').html(content.final_sum);//金额总计
                    var sell_price=$('.sell_price').attr('id');
                    if(countInputVal>=3){
                        var countsum=parseInt(countInputVal/3);
                        var discount_price=countsum*sell_price;
                        $('#discount_price').html(discount_price);//商品优惠
                        $('#sum_price').html(parseInt(sell_price*parseInt(countInputVal-countsum)));//金额总计
                        $('#youhui').html('满三减一');
                    }else{
                        $('#discount_price').html(content.reduce);//商品优惠
                        $('#sum_price').html(content.final_sum);//金额总计
