...
class System extends IController
{
    ...
    /*敏感词设置开始*/
    public function height_word(){
        $this->redirect('height_word');
    }
    //添加敏感词
    public function add_word(){
        $data['word']=$_POST['word'];
        $wordobj=new IModel('word');
        $wordobj->setData($data);
        $res=$wordobj->add();
        if($res){
            echo "<script>alert('设置敏感词成功');location.href=('system/conf_base')</script>";
        }
        else{
            echo "<script>alert('设置敏感词失败');location.href=('system/conf_base')</script>";
        }
			...
			case "index_slide":
			{
				...
				if(isset($_POST['slide_name']))
				{
					foreach($_POST['slide_name'] as $key => $value)
					{
                        ...
                        $cityObj=new IModel('ad_city');
                        $cityObj->setData($config_slide[$key]);
                        $cityObj->add();
					}
				}
