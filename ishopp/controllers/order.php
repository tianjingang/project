...
class Order extends IController
{
    ...
    /*设置单日最高消费金额开始*/
    function order_maxday(){
        $this->redirect('order_maxday');
    }
    function order_maxday_add(){
        $order_maxday=$_POST['order_maxday'];
        $ordermaxObj=new IModel('order_max');
        $data=array(
            "order_maxday"=>$order_maxday
        );
        $ordermaxObj->setData($data);
        $res=$ordermaxObj->add();
        if($res){
            $this->redirect('order_maxday');
        }
    ...
    public function order_count(){
       $order=new IQuery('order');//实例化订单表
        $arr=$order->find();//查询所有
        //print_r($arr);die;
        $data=array();//定义一个数组($data这是二维数组)
        foreach($arr as $key=>$val) {
            //print_r($val);die;
            $data[$key]['time'] = date('Y-m-d', strtotime($val['create_time']));//将下单时间转换为时间戳
            $data[$key]['pay_status'] = $val['pay_status'];//获取订单状态
           // print_r($data);die;
          foreach ($data as $key => $val) {
              //print_r($val);die;//打印出来是相同的
                foreach ($data as $k => $v) {
                   // print_r($v);die;//打印出来是相同的
                    if ($val['time'] == $v['time']) {
                        $data[$key]['zong'] += 1;
                       // print_r($data);die;
                        if ($val['pay_status'] == 1) {
