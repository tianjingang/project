...
class Member extends IController
{
    ...
    public function shop_list(){
       /* $search = IFilter::act(IReq::get('search'),'strict');
        ...
        $p_sum=$orderObj->query("user_id=2");//查询普通会员的总数
        $p_num=count($p_sum);//查询普通会员的总数
        $p_amount=$orderObj->query("user_id=2",'sum(real_amount) as p_amount');//查询普通会员的购买金额
        $pp_amount=$p_amount[0]['p_amount'];//查询普通会员的购买金额
        $V_sum=$orderObj->query("user_id=3");//查询vip会员的总数
        $V_num=count($V_sum);//查询vip会员的总数
        $V_amount=$orderObj->query("user_id=3",'sum(real_amount) as V_amount');//查询VIP会员的购买金额
        $VV_amount=$V_amount[0]['V_amount'];//查询VIP会员的购买金额
        $s_sum=$orderObj->query("seller_id!=0");//查询商家的总数
        $ss_num=count($s_sum);//查询商家的总数
        $s_amount=$orderObj->query("seller_id!=0",'sum(real_amount) as s_amount');//查询商家的购买金额
        $ss_amount=$s_amount[0]['s_amount'];//查询商家的购买金额
        $this->p_num=$p_num;
        $this->pp_amount=$pp_amount;
        $this->V_num=$V_num;
        $this->VV_amount=$VV_amount;
        $this->ss_num=$ss_num;
