...
class Simple extends IController
{
    ...
    //邀请注册
    function vip_reg(){
        $id     = IFilter::act(IReq::get('id','get'));
        $sign     = IFilter::act(IReq::get('sign','get'));
        $key='iwebshop';//定义一个加密字符串
        $keyid=$id*9;//加密
        $sign1=md5($key.$keyid);
        if($sign==$sign1){
            $userObj=new IModel('user');
            $arr=$userObj->getObj("id='$id'");
            //print_r($arr);die;
            $this->userinfo=$arr['username'];
            $this->userid=$id;
            $this->redirect('vip_reg');
        }else{
            echo "签名验证失败";die;
        }
    }
    ...
    function reg_act()
    {
        $reg_id= IFilter::act(IReq::get('reg_id','post'))?IFilter::act(IReq::get('reg_id','post')):1;;
    	...
    	$message    = '';
        $position   = IFilter::act(IReq::get('position','post'));
    ...
    function login_act()
    {
    	$login_info = IFilter::act(IReq::get('login_info','post'));//获取输入的用户名
    	$password   = IReq::get('password','post');//获取输入的密码
    	$remember   = IFilter::act(IReq::get('remember','post'));
    	$autoLogin  = IFilter::act(IReq::get('autoLogin','post'));
    	$callback   = IFilter::act(IReq::get('callback'),'text');
		$message    = '';
		$password   = md5($password);//把输入的密码md5加密
       /* $password   = md5(md5($password).'1410');//修改输入的密码加密方式
        $userObj=new IModel('user');
        $ar=$userObj->getObj("username='$login_info'");
        $pwd1=$ar['password'];
        //echo $pwd;die;
        $pwd2=md5($pwd1.'1410');
        if($password!=$pwd2){
    	...
    	else
    	{
    		...
    		{
                /*每天登陆登录+10积分开始*/
                $memberobj=new IQuery('member as m');//实例化积分表
                $memberobj->join="left join user as u on u.id=m.user_id";//两表联查
                $memberobj->where="u.username='$login_info' and u.password='$password'";//查询登陆者的
               $memberRow= $memberobj->find();//查询单表
                //var_dump($memberRow);die;
                $last_time=$memberRow[0]['last_login'];//获取上次登录时间
                $point=$memberRow[0]['point'];//获取金币
                $user_id=$memberRow[0]['user_id'];//获取用户user_id
                $last_time=strstr($last_time,"",true);//获取最后登录时间的年月日
                $dtime=date("Y-m-d",time());//获取当天时间
                if($last_time!=$dtime){
                     $arr=array(
                         'point'=>$point+10
                     );
                    $memberinfo=new IModel('member');//实例化积分表
                    $memberinfo->setData($arr);//需要变更的数据
                    $memberinfo->update("user_id='$user_id'");//修改
    ...
    function cart3()
    {
		...
		foreach($orderData as $seller_id => $goodsResult)
		{
           ...
           /*验证当日最高消费金额开始*/
            $order_max=new IModel('order_max');  //实例化单日最高消费金额表
            $re= $order_max->query();   //查询
            $order_maxday=$re[0]['order_maxday'];  //获取设定的单日最高消费金额
          //生成订单插入order表中
            $orderObj  = new IModel('order');   //实例化订单表
            $orderObj->setData($dataArray);
            $time=date("Y-m-d",time());  //获取当前系统时间
            $start_time =  $time.' 00:00:00';   //当前系统开始时间
            $end_time = $time.' 23:59:59';  //当前系统结束时间
            $orderObj  = new IModel('order');     //实例化订单表
            $ar=$orderObj->query("create_time>'$start_time' and create_time<'$end_time'",'SUM(real_amount) as amount');//查询order表
            $amount=$ar[0]['amount'];  //获取单日消费总额
            $chae=  $amount+ $dataArray['order_amount']-$order_maxday;     //计算差额
            if($chae<0){
                $order_id = $orderObj->add();
            }
            else{
                echo "对不起,超过当日最高消费金额.$order_maxday.无法下单,您当日已累计消费总额为.$order_maxday.,超出最高消费金额$chae";
                exit;
            }
             /*验证当日最高消费金额结束*/
            if($order_id == false)
...
/*			if( isset($dataRow['addtime']) && (time() - $dataRow['addtime'] <= 120) )
			...
			if($findPasswordDB->add())
			{
				...
				$result = Hsms::send($mobile,$mobile_code);
                //var_dump($result);die;
				if($result == 'success')
				{
					die('success');
