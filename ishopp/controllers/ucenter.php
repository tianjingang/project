...
/*        $email=IFilter::act( IReq::get('email','post') );*/
        $email=IFilter::act( IReq::get('email') );//被邀请人邮箱
        $user_id=$this->user['user_id'];//邀请人id
        $key='iwebshop';//定义一个加密字符串
        $keyid=$user_id*9;//加密
        $sign=md5($key.$keyid);
        $url="http://www.ishopp.com/index.php?controller=simple&action=vip_reg&id=$user_id&sign=$sign";
        $title='您的好友邀请您进入新世界公司';
        $content="猛戳此链接完成注册'$url'如果链接不可点击 复制到浏览器地址栏";
        $smtp  = new SendMail();
        $error = $smtp->getError();
        $to = $email;
        $bcc = array();
        $result = $smtp->send($to,$title,$content);
        if(!$result)
        {
            ...
            $user_id=$this->user['user_id'];//邀请人id
            $userObj=new IQuery('user');//实例化用户表
            $userObj->where="invite_id='$user_id'";
            $arr=$userObj->find();
            $sum=count($arr);
            $brr=$userObj->find("id='$user_id'");
            $this->redirect('invite');
        }
    }
    /*升级商家页面*/
    function seller(){
        $user_id=$this->user['user_id'];//登录者id
        $userObj=new IQuery('user');//实例化
        $userObj->where="invite_id='$user_id'";
        $arr=$userObj->find();
        $sum=count($arr);
        //echo $sum;die;
        if($sum>=3){
           ...
           // echo "<script>alert('欢迎进入升级商家界面');location.href='(ucenter/seller)'</script>";
        }
        else{
            echo "no";
            $this->redirect('invite');
            //echo "<script>alert('邀请好友成功注册成功至少三人才能有资格升级为商家');location.href='(ucenter/invite)'</script>";
        }
        //$this->redirect('seller');
    }
    //撤销评论
  public function delcomment(){
      $id=$_GET['id'];//获取id
      $commentobj=new IModel('comment');//实例化表
      $comment_row=$commentobj->getObj("id=".$id);//根据id查询单条
      $comment_time=$comment_row['comment_time'];//获取评论的时间
      $timestamp=strtotime($comment_time);//将评论时间转换为时间戳
      $date=time()-$timestamp;//获取时间戳
      if($date>3600){
          $res=$commentobj->del("id=".$id);
          if($res){
              echo "<script>alert('撤销成功');location.href='ucenter/isevaluation'</script>";
          }
          else{
              echo "<script>alert('撤销失败');location.href='ucenter/isevaluation'</script>";
          }

      }
      else{
          echo "<script>alert('对不起,发表的评论时间未过一小时无法撤销!');location.href='ucenter/isevaluation'</script>";
      }
