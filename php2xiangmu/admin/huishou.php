<div id="div1">
    ...
    $sqldel="update liuyan set l_status=1 where id='$id'";
    mysqli_query($link,$sqldel);
    //语句
    $sql="select count(*) num from liuyan where l_status=0";
    ...
    //语句
    $sql2="select * from liuyan  where l_status=0 limit $page_limit,$page_num";
                                    ...
                                    <table class="table table-striped table-bordered table-hover">
                                        ...
                                        <?php while($row=mysqli_fetch_assoc($res)){?>
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="liushow.php?id=<?php echo $row['id']?>"><i class="fa fa-user"></i>查看</a> |
                                                    <a href="javascript:void(0)" onclick="liu_del(<?php echo $page?>,<?php echo  $row['id']?>)"><i class="fa fa-times"></i>恢复</a>
                                                    | <a href="liuyong_del.php?id=<?php echo $row['id']?>">永久删除</a>
                                                </td>
                                            </tr>
                                        <?php } ?>
                                    </table>
    ...
    <script>
        ...
        function page(page){
            ...
            ajax.open('get','huishou.php?page='+page);
            ajax.send();
        ...
        function liu_del(page,id){
            var ajax=new XMLHttpRequest();
            ajax.open('get','huishou.php?page='+page+'&id='+id);
            ajax.send();
            ajax.onreadystatechange=function(){
                if(ajax.readyState==4&&ajax.status==200){
                    document.getElementById('div1').innerHTML=ajax.responseText;
